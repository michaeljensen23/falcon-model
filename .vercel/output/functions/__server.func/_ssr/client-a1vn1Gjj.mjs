...
//#region node_modules/.nitro/vite/services/ssr/assets/client-a1vn1Gjj.js
var client_a1vn1Gjj_exports = /* @__PURE__ */ __exportAll({
	i: () => signOut,
	n: () => client_exports,
	r: () => signIn,
	t: () => authClient
...
function isSessionBearer(token) {
	return token.length >= 16 && token.length <= 2048 && /^[\x21-\x7e]+$/.test(token);
}
/** The stored preview bearer token, or null. */
function getBearerToken() {
	if (typeof window === "undefined") return null;
	try {
		const token = window.sessionStorage.getItem(BEARER_KEY);
		if (!token || !isSessionBearer(token)) return null;
		return token;
	} catch {
		return null;
	}
...
function setBearerToken(token) {
	if (typeof window === "undefined") return;
	try {
		if (token && isSessionBearer(token)) window.sessionStorage.setItem(BEARER_KEY, token);
		else window.sessionStorage.removeItem(BEARER_KEY);
	} catch {}
}
var TRUSTED_SIGN_IN_HOSTS = /* @__PURE__ */ new Set([
	"auth.grok.me",
	"auth.grok.com",
	"auth.app-builder-testing.com",
	"accounts.google.com"
]);
/** Broker and Google only, or this app's own origin. Blocks a poisoned OAuth redirect. */
function isTrustedSignInUrl(location) {
	try {
		const url = new URL(location, window.location.origin);
		if (url.username || url.password) return false;
		if (url.origin === window.location.origin) return url.protocol === "https:" || url.protocol === "http:";
		if (url.protocol !== "https:") return false;
		return TRUSTED_SIGN_IN_HOSTS.has(url.hostname.toLowerCase());
	} catch {
		return false;
	}
}
...
/** Same-origin relative path only — blocks `//evil` and absolute open redirects. */
...
async function signIn(providerId, opts = {}) {
	const callbackURL = safeAppPath(opts.callbackURL);
	const errorCallbackURL = safeAppPath(opts.errorCallbackURL);
	...
	if (!data?.url || !isTrustedSignInUrl(data.url)) throw new Error("Sign-in failed");
...
function waitForPopupToken(popup) {
	return new Promise((resolve) => {
		...
		const onMessage = (event) => {
			...
			if (typeof token !== "string" || !isSessionBearer(token)) {
				settle(null);
				return;
			}
			settle(token);
...
async function signOut(redirectTo = "/") {
	await runSignOut({
		...
		clearToken: () => setBearerToken(null),
		redirect: () => {
			window.location.href = safeAppPath(redirectTo);
		}
	});
}
//#endregion
export { signOut as i, client_a1vn1Gjj_exports as n, signIn as r, authClient as t };
