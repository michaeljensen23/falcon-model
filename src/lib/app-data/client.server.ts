...
import { isSafeLoginUrl } from "./login.ts";
...
export function resolvePublicHost(
  ...
  const forwarded = hostOnly(
    typeof forwardedHost === "string" ? forwardedHost.split(",")[0] : forwardedHost,
  );
  const trustedPublic = (value: string | null): value is string =>
    Boolean(value && isTrustedPublicHost(value) && !isLoopbackHost(value));
...
function clampToolError(raw: string): string {
  const text = raw
    .replace(/[\u0000-\u001f\u007f]/g, " ")
    ...
    .trim()
    .slice(0, 240);
  return text || "connector request failed";
}
...
function unauthorizedResult(
  ...
  const fromJson =
    typeof json.loginUrl === "string" && isSafeLoginUrl(json.loginUrl)
      ? json.loginUrl
      : undefined;
...
function safeMemoKey(parts: unknown[]): string | null {
  try {
    ...
    if (!raw) return null;
    // Hash so a failed send does not retain the proposal PDF or client PII in the memo map.
...
export { isLoginRequired, isSafeLoginUrl, redirectToLoginIfRequired } from "./login.ts";
