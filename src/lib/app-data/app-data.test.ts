...
import { isLoginRequired, isSafeLoginUrl, redirectToLoginIfRequired } from "./login.ts";
...
describe("resolvePublicHost", () => {
  it("prefers a trusted public Host over a forwarded sibling", () => {
    assert.equal(
      resolvePublicHost("advisor.grok.me", "evil.grok.me"),
      "advisor.grok.me",
    );
    assert.equal(
      resolvePublicHost("advisor.grok.me:443", "evil.grok.me"),
      "advisor.grok.me",
    );
  });

  it("uses the forwarded host only when Host is loopback or untrusted", () => {
    assert.equal(
      resolvePublicHost("127.0.0.1:8080", "advisor.grok.me"),
      "advisor.grok.me",
    );
    assert.equal(
      resolvePublicHost("10.0.0.5", "preview.grok-sandbox.com"),
      "preview.grok-sandbox.com",
    );
  });

  it("rejects hosts that are not the gate's public suffixes", () => {
    assert.equal(resolvePublicHost("evil.example", "also.evil"), null);
    assert.equal(resolvePublicHost("notgrok.me", null), null);
    assert.equal(resolvePublicHost("advisor.grok.me.evil.com", null), null);
  });
});
...
describe("redirectToLoginIfRequired", () => {
  ...
  it("does not navigate to an untrusted loginUrl", () => {
    let target = "";
    const did = withWindow(
      {
        location: {
      ...
      () =>
        redirectToLoginIfRequired({
          ...
          data: null,
          loginRequired: true,
          loginUrl: "https://evil.example/phish",
        }),
    );
    assert.equal(did, false);
    assert.equal(target, "");
...
describe("isSafeLoginUrl", () => {
  it("allows the Grok connector gate and rejects everything else", () => {
    assert.equal(isSafeLoginUrl("https://gate.grok.me/__gate/signin?return_to=x"), true);
    assert.equal(isSafeLoginUrl("https://gate.app-builder-testing.com/__gate/signin"), true);
    assert.equal(isSafeLoginUrl("https://evil.example/phish"), false);
    assert.equal(isSafeLoginUrl("https://grok.me/login"), false);
    assert.equal(isSafeLoginUrl("javascript:alert(1)"), false);
    assert.equal(isSafeLoginUrl("http://gate.grok.me/__gate/signin"), false);
    assert.equal(isSafeLoginUrl("https://user:pass@gate.grok.me/__gate/signin"), false);
    assert.equal(isSafeLoginUrl("https://gate.evil.grok.me/__gate/signin"), false);
    assert.equal(isSafeLoginUrl("https://gate.grok.me.evil.com/__gate/signin"), false);
    assert.equal(isSafeLoginUrl("https://notgate.grok.me/__gate/signin"), false);
    assert.equal(isSafeLoginUrl("https://gate.grok.com/__gate/signin"), true);
  });
});
