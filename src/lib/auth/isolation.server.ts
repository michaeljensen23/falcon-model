...
export function assertSameSiteHeaders(headers: Headers, method: string): void {
  ...
  if (!site || site === "same-origin" || site === "none") {
    // Old browsers skip Sec-Fetch-Site. SameSite=Lax still sends cookies on
    // sibling-tenant requests, so Origin/Host must match whenever Origin is
    // present (GET list endpoints included) and always for mutating verbs.
    if (!site) {
...
function assertOriginMatchesHost(headers: Headers): void {
