...
function usableBearer(token: unknown): string | undefined {
  if (typeof token !== "string" || token.length < 16 || token.length > 2048) return undefined;
...
export const authMiddleware = createMiddleware({ type: "function" })
  ...
  .server(async ({ next, context }) => {
    ...
    const userId = await requireUserId(usableBearer(context.bearerToken));
